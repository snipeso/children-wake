...
CacheName = 'AllButAmplitudes.mat';
...
% fix y lims, so same for mor and eve
YLimits = [0 max(MetadataSim.Error)+.005; % error
    min(MetadataSim.RSquared)-.005 1.005; % r^2
    ];
XLim = [3 25];
...
for Idx1 = 1:numel(OutcomeMeasures)
    for Idx2 = 1:numel(OutcomeMeasures)
        ...
        plot_scattercloud(MetadataSim, OutcomeMeasures{Idx1}, OutcomeMeasures{Idx2}, PlotProps, '', false)
        set(gca, 'XTick' ,[], 'YTick', [])
        axis square
...
UniqueMetadata = unique_metadata(MetadataSim);
nParticipants = size(UniqueMetadata, 1);
UniqueMetadata.nRecordings = nan(nParticipants, 1);

for ParticipantIdx = 1:nParticipants
    Subset = MetadataSim.Index(strcmp(MetadataSim.Participant, UniqueMetadata.Participant{ParticipantIdx}));
...
load(fullfile(CacheDir, CacheName), 'MetadataSim')
Durations = MetadataSim;

CacheName = 'AllBursts.mat';
load(fullfile(CacheDir, CacheName), 'MetadataSim')
MetadataSim = basic_metadata_cleanup(MetadataSim);
MetadataSim.Dataset = Durations.Dataset; % recoded dataset names
MetadataSim.RecordingDuration = Durations.RecordingDuration;

% rename columns
OriginalTableLables = MetadataSim.Properties.VariableNames;
for Idx = 1:numel(OutcomeMeasuresTitles)

    IdxTable = strcmp(OriginalTableLables, OutcomeMeasures{Idx});
    MetadataSim.Properties.VariableNames(IdxTable) = genvarname(OutcomeMeasuresTitles(Idx));
end
...
writetable(MetadataSim, fullfile(Destination, 'Data1_AllWakeData.csv'))
