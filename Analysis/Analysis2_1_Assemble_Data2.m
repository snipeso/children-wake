...
Parameters.Tasks.BMS = {'1GoNoGo', '2Alertness'};
% Parameters.Tasks.BMSSL = {'1GoNoGoGopher', '3Alertness', '2Fixation'};
Parameters.Tasks.BMSSL = {'1GoNoGoGopher', '3Alertness'};
...
Metadata(strcmp(Metadata.Dataset, 'SleepLearning') & ...
    contains(Metadata.Session, {'Session_2', 'Session_3'}), :) = []; % remove year later followups
...
for RecordingIdx = 1:nRecordings
    ...
    for TaskIdx = 1:numel(Tasks)
        ...
        AllBursts = cat_struct(AllBursts, DataOut{1});
        AllBurstClusters = cat_struct(AllBurstClusters, DataOut{2});
        EEGMetadata = DataOut{3};
        TasksIncluded = cat(2, TasksIncluded, Task);
        ...
        nPoints = nPoints + EEGMetadata.pnts;
        ...
        DataOut = load_datafile(Path, Participant, Session, Hour, ...
            {'Power', 'Freqs'}, '.mat');
        Power = mean(cat(3, Power, DataOut{1}), 3);
        Freqs = DataOut{2};
    end
    ...
    TaskMetadata.Task{NewIdx} = TasksIncluded;
    TaskMetadata.Globality(NewIdx) = mean([AllBurstClusters.ClusterGlobality]);
    TaskMetadata.Amplitude(NewIdx) = mean([AllBurstClusters.ClusterAmplitude]);
    TaskMetadata.Duration(NewIdx) = mean([AllBurstClusters.ClusterEnd]-[AllBurstClusters.ClusterStart])/SampleRate; % burst durations

    TaskMetadata.Quantity(NewIdx) = 100*sum([AllBurstClusters.ClusterEnd]-[AllBurstClusters.ClusterStart])/nPoints; % number of bursts
    ...
    for ChannelIdx = 1:nChans
        for BandIdx = 1:nBands
            ...
            BurstsTemp = AllBursts(BurstChannels==ChannelIdx & ...
                [AllBursts.BurstFrequency]>=Band(1) & [AllBursts.BurstFrequency]<=Band(2));
    ...
    BurstFrequencies = discretize([AllBurstClusters.BurstFrequency], Frequencies);
    ...
    for FrequencyIdx = 1:nFrequencies
        ...
        BurstsTemp = AllBurstClusters(BurstIdx);
