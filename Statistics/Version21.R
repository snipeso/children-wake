...
# New function for all pairwise model comparisons with bootstrap
compare_all_models_bootstrap <- function(cv_results, n_repetitions, n_bootstrap = 1000, conf_level = 0.95) {
  
  # Extract models from results
  models <- unique(cv_results$fold_results$Model)
  n_models <- length(models)
  
  # Create results dataframe for all pairwise comparisons
  comparison_results <- data.frame(
    Model_A = character(),
    Model_B = character(),
    Mean_R2_Diff = numeric(),  # A - B
    CI_Lower = numeric(),
    CI_Upper = numeric(),
    Significant = logical()
  )
  ...
  for (i in 1:(n_models-1)) {
    ...
    for (j in (i+1):n_models) {
      ...
      # Step 1: Calculate observed difference at repetition level
      # (average across folds for each repetition to respect dependency structure)
      rep_data <- cv_results$fold_results %>%
        group_by(Repetition, Model) %>%
        summarize(Mean_R2 = mean(Predictive_R2), .groups = "drop") %>%
        filter(Model %in% c(model_a, model_b)) %>%
        pivot_wider(names_from = Model, values_from = Mean_R2) %>%
        mutate(diff = .data[[model_a]] - .data[[model_b]]) # A - B difference
      ...
      # Sample at repetition level, respecting the dependency structure
      n_rows <- nrow(rep_data)
      for (k in 1:n_bootstrap) {
        boot_sample <- sample(rep_data$diff, n_rows, replace = TRUE)
        boot_diffs[k] <- mean(boot_sample, na.rm = TRUE)
      }
  ...
  # Find the model with the highest overall mean R2
  best_model <- cv_results$summary %>%
    arrange(desc(Mean_Predictive_R2)) %>%
    pull(Model) %>%
    first()
...
# Modified plot function for all pairwise bootstrap comparisons
plot_all_bootstrap_diffs <- function(all_comparisons, outcome_name) {
  if (nrow(all_comparisons$comparisons) == 0) return(NULL)
  
  plot_data <- all_comparisons$comparisons
  
  # Create a label for each comparison
  plot_data <- plot_data %>%
    mutate(Comparison = paste(Model_A, "vs", Model_B))
  
  p <- ggplot(plot_data, aes(x = Comparison, y = Mean_R2_Diff)) +
    geom_point(size = 3) +
    geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.2) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
    # Add highlight for significant comparisons
    geom_point(data = plot_data %>% filter(Significant), 
               aes(x = Comparison, y = Mean_R2_Diff),
               size = 4, shape = 8, color = "red") +
    labs(
      title = paste("Pairwise Bootstrap Comparisons -", outcome_name),
      subtitle = paste("Model A - Model B (Red stars indicate significant differences)"),
      x = "Model Comparison",
      y = "Difference in R² (A - B)"
    ) +
    theme_light() +
...
# Create a summary table for all pairwise comparisons
summarize_pairwise_comparisons <- function(all_comparisons, cv_results) {
  # Get mean R2 values for each model
  r2_means <- cv_results$summary %>%
    select(Model, Mean_Predictive_R2) %>%
    rename(R2 = Mean_Predictive_R2)
  
  # Create a matrix for pairwise comparison results
  models <- unique(cv_results$fold_results$Model)
  n_models <- length(models)
  
  # Initialize matrix with NA values
  result_matrix <- matrix(NA, nrow = n_models, ncol = n_models)
  rownames(result_matrix) <- models
  colnames(result_matrix) <- models
  
  # Fill in the matrix with comparison results
  for (i in 1:nrow(all_comparisons$comparisons)) {
    row <- all_comparisons$comparisons[i, ]
    model_a <- row$Model_A
    model_b <- row$Model_B
    
    # Format result as mean difference [CI lower, CI upper] with * for significance
    result_text <- sprintf("%.3f [%.3f, %.3f]%s", 
                           row$Mean_R2_Diff, 
                           row$CI_Lower, 
                           row$CI_Upper,
                           ifelse(row$Significant, "*", ""))
    
    # Store results in both cells (with appropriate sign)
    result_matrix[model_a, model_b] <- result_text
    # For the opposite comparison, negate the values
    neg_diff <- -row$Mean_R2_Diff
    neg_ci_upper <- -row$CI_Lower  # Note: lower becomes upper when negated
    neg_ci_lower <- -row$CI_Upper  # Note: upper becomes lower when negated
    result_matrix[model_b, model_a] <- sprintf("%.3f [%.3f, %.3f]%s", 
                                               neg_diff, 
                                               neg_ci_lower, 
                                               neg_ci_upper,
                                               ifelse(row$Significant, "*", ""))
  ...
  # Add diagonal (model vs. itself) as zeros
  for (model in models) {
    result_matrix[model, model] <- "0.000 [0.000, 0.000]"
  }
  
  # Convert to a data frame for better display
  result_df <- as.data.frame(result_matrix)
  
  # Add R2 means to the result data frame
  result_df$Mean_R2 <- r2_means$R2[match(rownames(result_df), r2_means$Model)]
  
  # Return both the matrix and more structured results
  return(list(
    matrix = result_df,
    best_model = all_comparisons$best_model,
    comparisons = all_comparisons$comparisons
  ))
}

# Function to create a heatmap of significance pattern
plot_comparison_heatmap <- function(all_comparisons, cv_results, outcome_name) {
  # Get all models
  models <- unique(cv_results$fold_results$Model)
  
  # Create a grid of all pairwise comparisons
  comparison_grid <- expand.grid(Model_A = models, Model_B = models) %>%
    filter(Model_A != Model_B) %>%
    mutate(Key = paste(pmin(Model_A, Model_B), pmax(Model_A, Model_B), sep = "_"))
  
  # Add R2 difference and significance from the comparisons
  merged_data <- comparison_grid %>%
    left_join(
    ...
    mutate(
      # Adjust for comparison direction
      Adjusted_Diff = ifelse(
        Model_A == Model_A.y & Model_B == Model_B.y,
        Mean_R2_Diff,
        -Mean_R2_Diff
      ),
      Significance = ifelse(Significant, "Significant", "Not Significant"),
      # Make sure the magnitude of difference is preserved
      Magnitude = abs(Mean_R2_Diff)
    )
  
  # Get model order based on mean R2
  model_order <- cv_results$summary %>%
    arrange(desc(Mean_Predictive_R2)) %>%
    pull(Model)
  
  # Create a heatmap of R2 differences
  p <- ggplot(merged_data, aes(x = factor(Model_A, levels = model_order), 
                               y = factor(Model_B, levels = model_order), 
                               fill = Adjusted_Diff)) +
    geom_tile() +
    scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0,
                         name = "R² Difference\n(row - column)") +
    # Add significance marker
    geom_text(aes(label = ifelse(Significant, "*", "")), size = 8) +
    labs(
      title = paste("Pairwise Comparison Heatmap -", outcome_name),
      subtitle = "* indicates significant difference",
      x = "Model A",
      y = "Model B"
    ) +
    theme_light() +
...
# Modified section of your script with all pairwise comparisons
...
# Run all pairwise bootstrap comparisons
message("Performing all pairwise bootstrap comparisons for Sleep Slope...")
slope_all_comparisons <- compare_all_models_bootstrap(
  cv_slope_results, n_repetitions, n_bootstrap, conf_level
)

message("Performing all pairwise bootstrap comparisons for Sleep Amplitude...")
amp_all_comparisons <- compare_all_models_bootstrap(
  cv_amp_results, n_repetitions, n_bootstrap, conf_level
)

# For backward compatibility, also run the original bootstrap comparisons
message("Performing original bootstrap comparisons for Sleep Slope...")
slope_comparisons <- compare_to_best_model(
  cv_slope_results, n_repetitions, n_bootstrap, conf_level
)

message("Performing original bootstrap comparisons for Sleep Amplitude...")
...
# Create summary tables for all pairwise comparisons
slope_pairwise_summary <- summarize_pairwise_comparisons(slope_all_comparisons, cv_slope_results)
amp_pairwise_summary <- summarize_pairwise_comparisons(amp_all_comparisons, cv_amp_results)

# Write pairwise comparison tables to CSV
write.csv(slope_pairwise_summary$matrix, "slope_pairwise_comparisons.csv", row.names = TRUE)
write.csv(amp_pairwise_summary$matrix, "amplitude_pairwise_comparisons.csv", row.names = TRUE)

# Print pairwise comparison matrices
message("\n==== Pairwise Comparison Matrix for Sleep Slope ====\n")
print(slope_pairwise_summary$matrix)

message("\n==== Pairwise Comparison Matrix for Sleep Amplitude ====\n")
print(amp_pairwise_summary$matrix)
...
# Create bootstrap visualization plots for original comparisons
...
# Create all pairwise bootstrap comparison plots
message("Creating all pairwise bootstrap comparison plots...")
slope_all_bootstrap_plot <- plot_all_bootstrap_diffs(slope_all_comparisons, "Sleep Slope")
amp_all_bootstrap_plot <- plot_all_bootstrap_diffs(amp_all_comparisons, "Sleep Amplitude")

# Create heatmap visualizations
message("Creating heatmap visualizations...")
slope_heatmap <- plot_comparison_heatmap(slope_all_comparisons, cv_slope_results, "Sleep Slope")
amp_heatmap <- plot_comparison_heatmap(amp_all_comparisons, cv_amp_results, "Sleep Amplitude")

# Print summary of key findings
message("\n==== Summary of Statistical Comparison Results ====\n")

message("Best predictor for Sleep Slope: ", slope_all_comparisons$best_model)
message("Number of significant pairwise differences: ", 
        sum(slope_all_comparisons$comparisons$Significant))

message("\nBest predictor for Sleep Amplitude: ", amp_all_comparisons$best_model)
message("Number of significant pairwise differences: ", 
        sum(amp_all_comparisons$comparisons$Significant))

message("\nAll pairwise comparison details for Sleep Slope:")
print(slope_all_comparisons$comparisons %>% 
        select(Model_A, Model_B, Mean_R2_Diff, CI_Lower, CI_Upper, Significant))

message("\nAll pairwise comparison details for Sleep Amplitude:")
print(amp_all_comparisons$comparisons %>% 
        select(Model_A, Model_B, Mean_R2_Diff, CI_Lower, CI_Upper, Significant))

# Save results for further analysis
save(cv_slope_results, cv_amp_results, 
     slope_comparisons, amp_comparisons,
     slope_all_comparisons, amp_all_comparisons,
     slope_effect_sizes, amp_effect_sizes, 
     slope_models, amp_models,
     slope_pairwise_summary, amp_pairwise_summary,
     file = "all_pairwise_model_comparison_results.RData")