...
cross_validate_models <- function(data, outcome_var, k=10) {
  ...
  for (i in 1:k) {
    ...
    # For linear models
    lm_formula1 <- as.formula(paste(outcome_var, "~ Age + Amplitude"))
    lm_formula2 <- as.formula(paste(outcome_var, "~ Age + Duration"))
    lm_formula3 <- as.formula(paste(outcome_var, "~ Age + Offset"))
    lm_formula4 <- as.formula(paste(outcome_var, "~ Age + Exponent"))
...
create_summary <- function(cv_results, me_rsq_data, title) {
  cat("\n===== SUMMARY OF", title, "RESULTS =====\n")
  
  # Create mean R-squared values table
  summary_table <- data.frame(
    Predictor = c("Amplitude", "Duration", "Offset", "Exponent"),
    Linear_CV_Rsq = colMeans(cv_results$lm),
    Mixed_Marginal_Rsq = me_rsq_data$Marginal_R2,  # From r.squaredGLMM
    Mixed_Conditional_CV_Rsq = colMeans(cv_results$me)
  )
  
  # Sort by Mixed Marginal (most relevant for comparing predictors)
  summary_table <- summary_table[order(-summary_table$Mixed_Marginal_Rsq),]
  print(round(summary_table, 4))
  
  # Identify best predictor for each model type
  cat("\nBest predictor by model type:\n")
  cat("Linear model (CV):", summary_table$Predictor[which.max(summary_table$Linear_CV_Rsq)], "\n")
  cat("Mixed model (marginal R²):", summary_table$Predictor[which.max(summary_table$Mixed_Marginal_Rsq)], "\n")
  cat("Mixed model (conditional CV R²):", summary_table$Predictor[which.max(summary_table$Mixed_Conditional_CV_Rsq)], "\n")
  
  return(summary_table)
}

# Create summaries
slope_summary <- create_summary(cv_slope, me_rsq_slope, "SLEEP SLOPE")
amp_summary <- create_summary(cv_amp, me_rsq_amp, "SLEEP AMPLITUDE")