...
# Modified cross-validation function with repetitions
cross_validate_mixed_models <- function(data, outcome_var, k=10, repeats=5) {
  ...
  for(rep in 1:repeats) {
    # Create participant-based folds with a different seed for each repeat
    set.seed(123 + rep)  # Different seed for each repetition
    ...
    for (i in 1:k) {
      ...
      for (pred in predictors) {
        ...
        tryCatch({
          ...
          warning(paste("Error in repeat", rep, "fold", i, "with predictor", pred, ":", e$message))
        })
      }
    }
  }
